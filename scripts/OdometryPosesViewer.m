...
if strcmp(cPlotType,'ONE')
    ...
    for i = 1 : odometryMappingSize
        ...
        if isfolder(tRawDataFolderPath)
            ...
            cOxtslitePoseMatFilePath = fullfile(tRawDataFolderPath,cOxtsFolderName,cOxtslitePoseMatFileName);
            if ~isfile(cOxtslitePoseMatFilePath) || cRecomputeOxtslitePoseMatFile == 1
                tOxtslitePoseRaw = convertOxtsToWorldPose(tOxtsliteData);
                tOxtsliteTimestamps = loadOxtsTimestamps(tRawDataFolderPath);
                
                tOxtsliteTimestampsSize = size(tOxtsliteTimestamps,1);
                tOxtslitePose = cell(tOxtsliteTimestampsSize,2);
                for j = 1:tOxtsliteTimestampsSize
                    tOxtslitePose{j,1} = tOxtsliteTimestamps(j);
                    tOxtslitePose{j,2} = tOxtslitePoseRaw{j};
                    tOxtslitePose{j,3} = tOxtsliteData{j}([8 7 11]);
                    tOxtslitePose{j,4} = tOxtsliteData{j}(21:23);
                    tOxtslitePose{j,5} = tOxtsliteData{j}(15:17);
                end
